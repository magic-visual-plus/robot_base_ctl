...
class Ds402StateMachine:
    ...
    def enable_operation(self, timeout_s: float = 5.0) -> None:
        """
        CiA402 上电使能序列。若 init/驱动侧已处于较高状态（常见于已 OPERATION_ENABLED），
        不再强行从 0x0006 等 RTSO，避免超时（例如 SW=0x1637 已是运行态仍等 READY_TO_SWITCH_ON）。
        """
        st = decode_ds402_state(self.io.read_statusword())
        if st == Ds402State.OPERATION_ENABLED:
            return
        if st == Ds402State.SWITCHED_ON:
            self.io.write_controlword(0x000F)
            self.wait_state(Ds402State.OPERATION_ENABLED, timeout_s=timeout_s)
            return
        if st == Ds402State.READY_TO_SWITCH_ON:
...
class CanopenBus:
    ...
    def connect(self, can_filters: Optional[List[Dict]] = None) -> None:
        ...
        if self.auto_sync:
            try:
                self.net.sync.start(self.sync_period_s)
            except Exception as e:
                LOG.warning("net.sync.start(%s) failed: %s", self.sync_period_s, e)

        LOG.info(
            "Connected: interface=%s channel=%s bitrate=%s auto_sync=%s sync_period_ms=%.3f",
            self.interface,
            self.channel,
            self.bitrate,
            self.auto_sync,
            self.sync_period_s * 1000.0,
        )
...
class CanopenDs402IO(Ds402IO):
    ...
    def configure_rpdo_sync(
        self,
        rpdo_num: int,
        trans_type: int = 1,
        enable: bool = True,
    ) -> None:
        pdo = self.node.rpdo[rpdo_num]
        pdo.enabled = enable
        try:
            pdo.trans_type = trans_type
        except Exception:
    ...
    def init(self, default_mode: ControlModeName = "PV", zero_on_init: bool = True) -> None:
        ...
        try:
            ...
            self.io.configure_tpdo_limit_rate(
                ...
                enable=True,
                trans_type=255,
                event_timer_ms=cfg.tpdo_event_timer_ms,
                inhibit_time_100us=cfg.tpdo_inhibit_time_100us,
            )
            self.io.configure_tpdo_limit_rate(
                cfg.tpdo_fb_num,
                enable=True,
        ...
        # RPDO 命令侧同步：缓冲在 SYNC 时写入应用（见 MDX+ 手册 §3.7.2 RxPDO 0~240）
        try:
            cfg = self.io.cfg
            self.io.configure_rpdo_sync(
                cfg.rpdo_vel_num,
                trans_type=cfg.rpdo_vel_trans_type,
                enable=True,
            )
            try:
                self.io.configure_rpdo_sync(
                    cfg.rpdo_pos_num,
                    trans_type=cfg.rpdo_pos_trans_type,
                    enable=True,
                )
            except Exception:
                pass
        except Exception as e:
            LOG.warning("RPDO sync config failed axis=%d: %s", self.axis_id, e)
