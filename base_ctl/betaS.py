...
CMD_TOPIC  = "/cmd_vel"
PLAYBACK_SCALE = 0.5   # 0.5 = 慢放2倍；0.25 = 慢放4倍

CONTROL_HZ = 30           # 补帧控制频率（30/60都行，建议60）
    ...
    def run(self):
        ...
        try:
            while rclpy.ok():
                ...
                # ---- continuous reference time (no catch-up stepping)
                t_ref = now - self.start_t            # seconds since start
                k = int(t_ref / DT_KEY) + 1           # idx uses (idx-1, idx), and idx starts at 1

                # end condition
                if k >= len(self.dataset) or k >= MAX_FRAMES:
                ...
                # alpha within this keyframe interval
